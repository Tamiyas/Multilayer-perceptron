...
class Mirror():
  def __init__(self, column = 6):
    ...
    Attributes:
      ...
      train:  教師データ.
    """
    self.data = self.generate_mirror(column)
  ...
  def generate_mirror(self, column):
    ...
    Returns:
      2進数列
    """
    # 2進数をdataに格納(数列の先頭を0パディングする)
    data = [(format(i, 'b').zfill(column)) for i in range(2 ** column)]
    self.str_data = data
    return np.array([list(map(int, X)) for X in data])
  ...
  def check_symmetry(self):
    ...
    train = np.zeros((self.data.shape[0], 1)).astype('int')
    for (i, X) in enumerate(self.str_data):
      # 文字列の前半部分を取得する
      first = X[:center]
      # 文字列の後半部分を取得する
      last = X[center:]
